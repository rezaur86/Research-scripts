...
each_app_anonym = each_app + '_sampled_RW'
try:
    for user_id in csv.reader(open(sys.argv[3])):
        sampled_seeds.add(int(user_id))
except:
    ...
    for user_id,odeg,indeg,deg in csv.reader(open('/home/rezaur/data/iheart_ext_preprocessed_basic.txt')):
        ...
        degree[int(user_id)] = int(deg)
    print "Sampling..."
    initial_nodes = set(WeightedSelectionWithoutReplacement(weights, SAMPLE_SIZE))
    nodes_to_visit_next = copy.deepcopy(initial_nodes)
    w = csv.writer(open("/home/rezaur/data/sampled_users/RW.txt", "w"))
    for user_id in initial_nodes:
        w.writerow([user_id]) 
    print "Sampling Done"
#     non_zero_odeg_users = []
#     for user_id,odeg,indeg,deg in csv.reader(open('/home/rezaur/data/iheart_ext_preprocessed_basic.txt')):
#         if int(odeg) > 0:
#             senders[int(user_id)] = int(odeg)
#             non_zero_odeg_users.append(int(user_id))
#     total_senders = len(non_zero_odeg_users)
#     print "Sampling Seeds..."
#     sampled_seeds = set(sample(non_zero_odeg_users, int(total_senders*.25)))
#     nodes_to_visit_next = copy.deepcopy(sampled_seeds)
#     w = csv.writer(open("/home/rezaur/data/sampled_users/RW.txt", "w"))
#     for user_id in sampled_seeds:
#         w.writerow([user_id]) 
#     print "Sampling Done"
...
for each_file in file_list:
    ...
    for line in lines:
        ...
        if sender_org_id in initial_nodes and sender_org_id not in nodes_to_visit_next:
            # Returning to the initial node randomly
            if random.random() <= .8:
                nodes_to_visit_next.add(sender_org_id)
                take_this_act = True                
        if recv_org_id in initial_nodes and recv_org_id not in nodes_to_visit_next:
            # Returning to the initial node randomly
            if random.random() <= .8:
                nodes_to_visit_next.add(recv_org_id)
                take_this_act = True                
        if sender_org_id in nodes_to_visit_next:
            if random.random()*degree[sender_org_id] <= 1.0:
                nodes_to_visit_next.remove(sender_org_id)
                nodes_to_visit_next.add(recv_org_id)
                take_this_act = True
        if recv_org_id in nodes_to_visit_next:
            if random.random()*degree[recv_org_id] <= 1.0:
