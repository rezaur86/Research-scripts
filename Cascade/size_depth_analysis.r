...
#	print (nrow(depth_expansion.df[depth_expansion.df$depth==0,]))
	...
	plot <- ggplot(depth_expansion.df, aes(x = depth, y = (diff))) + geom_line(aes(group = root_user_id,colour = root_user_id)) + xlab('Depth') + ylab('Shell size growth') +
			scale_colour_hue(name  ="Top influencer")
